...
def get_peft_model_state_dict(model, state_dict=None):
    ...
    elif model.peft_config.peft_type == PeftType.CONTROL:
        print("Control Saving")
        bias = model.peft_config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "lora_" in k or "weight_m_wdecomp" in k}
        elif bias == "all":
...
def set_peft_model_state_dict(model, peft_model_state_dict):
    ...
    if model.peft_config.peft_type != PeftType.LORA and model.peft_config.peft_type != PeftType.CONTROL and model.peft_config.peft_type != PeftType.BOTTLENECK and model.peft_config.peft_type != PeftType.DORA:
