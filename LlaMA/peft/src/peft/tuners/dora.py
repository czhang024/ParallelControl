...
class DoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, model):
        super().__init__()
        self.additional_peft_config = config
        self.model = model
        self._find_and_replace()
        mark_only_lora_as_trainable(self.model, self.additional_peft_config.bias)
    ...
    def _find_and_replace(self):
        ...
        kwargs = {
            "r": self.additional_peft_config.r,
            "lora_alpha": self.additional_peft_config.lora_alpha,
            "lora_dropout": self.additional_peft_config.lora_dropout,
            "fan_in_fan_out": self.additional_peft_config.fan_in_fan_out,
            "merge_weights": (self.additional_peft_config.merge_weights or self.additional_peft_config.inference_mode)
            and not is_hf_device_map_available,
            "dora_simple": self.additional_peft_config.dora_simple
        }
        key_list = [key for key, _ in self.model.named_modules()]
        for key in key_list:
            if isinstance(self.additional_peft_config.target_modules, str):
                target_module_found = re.fullmatch(self.additional_peft_config.target_modules, key)
            else:
                target_module_found = any(key.endswith(target_key) for target_key in self.additional_peft_config.target_modules)

            if isinstance(self.additional_peft_config.Wdecompose_target_modules, str):
                wdecompose_target_module_found = re.fullmatch(self.additional_peft_config.Wdecompose_target_modules, key)
            elif self.additional_peft_config.Wdecompose_target_modules == None:
                wdecompose_target_module_found = False
            else:
                wdecompose_target_module_found = any(key.endswith(target_key) for target_key in self.additional_peft_config.Wdecompose_target_modules)
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    if self.additional_peft_config.enable_lora is None:
                        new_module = Linear8bitLt(target.in_features, target.out_features, bias=bias, **kwargs)
                    else:
                        raise NotImplementedError
                    
                elif isinstance(target, torch.nn.Linear) and self.additional_peft_config.enable_lora is None:
                    new_module = Linear(target.in_features, target.out_features, bias=bias, **kwargs)
                elif self.additional_peft_config.enable_lora is not None:
                    raise NotImplementedError
        ...
        if not is_target_modules_in_base_model:
            raise ValueError(
                f"Target modules {self.additional_peft_config.target_modules} not found in the base model. "
    ...
    def get_peft_config_as_dict(self, inference: bool = False):
        config = {k: v.value if isinstance(v, Enum) else v for k, v in asdict(self.additional_peft_config).items()}
        if inference:
