...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
        ...
        - **additional_peft_config** ([`PeftConfig`]) -- The configuration of the additional Peft model (e.g., additional LoRA).
    ...
    def __init__(self, model, peft_config: PeftConfig):
        super().__init__()
        self.additional_peft_config = peft_config  # remove this line to avoid conflict with control
        self.base_model = model
        self.config = self.base_model.config
        self.modules_to_save = None
        if isinstance(self.additional_peft_config, PromptLearningConfig):
            self._setup_prompt_encoder()
        else:
            if self.additional_peft_config.peft_type == PeftType.LORA:
                self.base_model = LoraModel(peft_config, model)
            elif self.additional_peft_config.peft_type == PeftType.BOTTLENECK:
                self.base_model = BottleneckModel(peft_config, model)
            elif self.additional_peft_config.peft_type == PeftType.DORA:
